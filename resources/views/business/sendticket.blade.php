        ...
        <div class="card-body">
                @foreach($tickets as $ticket)
                     <div class="p-0 alert @if($ticket->user_id) alert-primary @else alert-success @endif">
                        @if($ticket->user_id)
                            <div class="bg-primary text-white p-1 pl-3 pr-3 mb-1 rounded-top">{{ Auth::user()->username }} <span>{{ $ticket->message_status }}</span> <span class="float-right">{{ $ticket->created_at->diffForHumans() }}</span></div>
                        @else
